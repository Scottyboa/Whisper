...
// --- Experiment toggle: use Voxtral Small via chat ---
const USE_VOXTRAL_SMALL = true;
...
// Direct call to Mistral audio transcriptions when not using chat+audio
async function sendChunkToMistralTranscribe(wavBlob, chunkNum, { language = null } = {}) {
  ...
  const resp = await fetch(`${MISTRAL_BASE}/audio/transcriptions`, {
    method: "POST",
...
async function transcribeChunkDirectly(wavBlob, chunkNum) {
  ...
  try {
    if (USE_VOXTRAL_SMALL) {
      // Mistral chat + audio
      const audio_b64 = await blobToBase64(wavBlob);
      const payload = {
        model: "voxtral-small-latest", // or "voxtral-mini-latest"
        temperature: 0.2,
        messages: [{
          role: "user",
          content: [
            ...
            { type: "text", text:
              "Transcribe the audio in the detected language. If Danish is detected, output Norwegian Bokmål; otherwise use the detected language."
            }
          ]
        }]
      };
      ...
      // Mistral STT endpoint
      return await sendChunkToMistralTranscribe(wavBlob, chunkNum);
    }
  } catch (error) {
    logError(`Error transcribing chunk ${chunkNum}:`, error);
    updateStatusMessage("Transcription error", "red");
